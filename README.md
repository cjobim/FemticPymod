#femticPymod is a fork of femticPy by Hoël Seillé (https://github.com/hseille/femticPy) that includes some modifications I made for my personal use over the course of my PhD and does not mean to steal the thunder of the original, it is mainly a convenient place to put this additional code while waiting for elements of it to be brought back into the original code/reworked and/or for people with similar needs to try it out. 

A quick summary of the modifications are: 
- allowing the use of different ways to parameter data errors for different data types
- more flexible plotting of induction arrows
- caching of the background DEM used for most maps, which saves (computation) time.
- a modified data plotting routine for QA with full impedance plots of input data and forward-modelled data with tippers on the side, a map and RMS for each data type. shaded data error envelopes and error bars.
- an integrated QC routine that handles the merging of femtic results and plotting outputs in one go.
...
Below is the original decription of femticPy
