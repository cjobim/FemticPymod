...
proj_distance=0.8   #in km
nbprofiles=4    #number of profiles

df_loc = pd.read_csv(file_loc, delim_whitespace=True, names=['x', 'y', 'z', 'station_name']) #all in model space, in km, ie x=northing in general
...
with open(file_nodes, "r") as infile_nodes:
    ...
    for i in range(nbprofiles):
        ...
        profile_stations=[]
        filename_profile_stations = f"{p_name}_stations.csv"
        
        for row in df_loc.itertuples(index=False):
            dist = distance_to_profile(row.x,row.y,p1_x,p1_y,p2_x,p2_y)
            if dist < proj_distance:
                x_profile = distance_along_profile(row.x,row.y,p1_x,p1_y,p2_x,p2_y)
